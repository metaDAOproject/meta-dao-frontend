...
export function ConditionalMarketCard({
  ...
  const [observedTimestamp, setObservedTimestamp] = useState<number>(0);
    ...
    const maxOrderAmount = () => {
        ...
        if (quoteBalance && price) {
            const _maxAmountRatio = Math.floor(
                Number(quoteBalance?.uiAmountString) / Number(price),
            );
    ...
    const isOrderAmountNan = () => {
        const _orderAmount = numeral(maxOrderAmount()).format(
            isAskSide ? BASE_FORMAT : NUMERAL_FORMAT,
        );
    ...
    const isWinning = () => {
        if (passTwap && failTwap && daoState) {
            ...
            if (isPassMarket)
                return passWin || proposal?.account.state.passed ? '#67BD63' : 'inherit';
        ...
        >
            ...
            <Stack gap="xs">
                <Group justify="space-between" align="flex-start">
                    ...
                    <Tooltip
                        label="Crank the market 🐷"
                        events={{ hover: true, focus: true, touch: false }}
                    >
                        <ActionIcon
                            variant="subtle"
                            loading={isCranking}
                            onClick={() => crankMarkets()}
                        >
                            <Icon12Hours />
                        </ActionIcon>
                    </Tooltip>
                ...
                {twap ? (
                    <Group justify="center" align="center">
                        <Stack gap={0} pb="1rem" align="center">
                            ...
                            <Text size="xs">
                                $
                                {numeral(isPassMarket ? passMidPrice : failMidPrice).format(
                                    NUMERAL_FORMAT,
                                )}{' '}
                                (mid)
                            </Text>
                        ...
                        <HoverCard position="top">
                            ...
                            <HoverCard.Dropdown w="22rem">
                                <Stack>
                                    <Text>
                                        The Time Weighted Average Price (TWAP) is the measure used
                                        to decide if the proposal passes: if the TWAP of the pass
                                        market is{' '}
                                        {daoState
                                            ? `${numeral(daoState.passThresholdBps / 100).format(
                                                  NUMERAL_FORMAT,
                                              )}%`
                                            : '???'}{' '}
                                        ...
                                        {daoState && failTwap
                                            ? `(> ${numeral(
                                                  (failTwap * (10000 + daoState.passThresholdBps)) /
                                                      10000,
                                    ...
                                    <Text size="xs">
                                        ...
                                        >
                                            {`See ${
                                                isPassMarket ? 'Pass' : 'Fail'
                                            } TWAP Market in explorer.`}
                                        </a>
                                    </Text>
                                </Stack>
                            </HoverCard.Dropdown>
                                ...
                                <Text size="xs">
                                    {isAskSide
                                        ? `${isPassMarket ? 'p' : 'f'}META ${
                                              numeral(baseBalance?.uiAmountString || 0).format(
                                                  BASE_FORMAT,
                                              ) || ''
                                          }`
                                        : `${isPassMarket ? 'p' : 'f'}USDC $${
                                              numeral(quoteBalance?.uiAmountString || 0).format(
                                                  NUMERAL_FORMAT,
                                              ) || ''
                                          }`}
                                </Text>
